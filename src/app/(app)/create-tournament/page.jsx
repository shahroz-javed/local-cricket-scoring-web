          ...
          <div key={i} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all
                ${done   ? "cricket-gradient border-primary text-white"
            ...
            {i < STEPS.length - 1 && (
              <div className={`flex-1 h-0.5 mx-2 mb-4 ${done ? "bg-primary" : "bg-outline-variant"}`} />
            )}
          </div>
        );
      })}
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-3">
          {FORMAT_OPTIONS.map(opt => (
            ...
            >
              ...
              {data.format === opt.value && (
                <Icon name="check_circle" className="text-primary ml-auto text-xl shrink-0" />
              )}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Match Type">
          <select
            ...
            onChange={e => setData(d => ({ ...d, match_type: e.target.value }))}
        ...
        <Field label="Overs per Match">
          <Input
            ...
            onChange={e => setData(d => ({ ...d, overs_limit: parseInt(e.target.value) || 20 }))}
          />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Start Date" hint="Optional">
          <Input
            ...
            onChange={e => setData(d => ({ ...d, starts_at: e.target.value }))}
          />
        </Field>
        <Field label="End Date" hint="Optional">
          <Input
            type="date"
            value={data.ends_at}
            onChange={e => setData(d => ({ ...d, ends_at: e.target.value }))}
          />
        </Field>
      </div>
      ...
      <Field label="Default Venue" hint="Applied to all fixtures unless overridden.">
        <Input
          ...
          onChange={e => setData(d => ({ ...d, default_venue: e.target.value }))}
          maxLength={150}
        />
      </Field>
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-outline-variant last:border-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="w-6 text-center font-bold text-foreground text-sm">{value}</span>
    ...
    <div className="space-y-6">
      {showLeagueOptions && (
        <div className="bg-white border border-outline-variant rounded-2xl p-5">
          ...
          <PointsInput label="Win"       value={data.points_win}       onChange={v => setData(d => ({ ...d, points_win: v }))} />
          <PointsInput label="Tie"       value={data.points_tie}       onChange={v => setData(d => ({ ...d, points_tie: v }))} />
          <PointsInput label="No Result" value={data.points_no_result} onChange={v => setData(d => ({ ...d, points_no_result: v }))} />
          <PointsInput label="Loss"      value={data.points_loss}      onChange={v => setData(d => ({ ...d, points_loss: v }))} />
        </div>
      )}
      ...
      {showLeagueOptions && (
        <div className="bg-white border border-outline-variant rounded-2xl p-5">
          ...
          <p className="text-xs text-foreground-muted mb-4">Select which tiebreakers apply, in priority order (top = highest priority).</p>
          <div className="space-y-2">
            {TIEBREAKERS.map(tb => {
              const active = (data.tiebreaker_rules || []).includes(tb.key);
              const rank   = (data.tiebreaker_rules || []).indexOf(tb.key) + 1;
              return (
                <button key={tb.key} type="button" onClick={() => toggleTiebreaker(tb.key)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all text-left
                    ${active ? "border-primary bg-primary/5" : "border-outline-variant bg-surface-low hover:border-primary/30"}`}>
                  <Icon name={tb.icon} className={`text-lg ${active ? "text-primary" : "text-foreground-muted"}`} />
                  <span className={`flex-1 text-sm font-medium ${active ? "text-foreground" : "text-foreground-muted"}`}>{tb.label}</span>
                  {active && <span className="text-xs font-bold text-white bg-primary rounded-full w-5 h-5 flex items-center justify-center">{rank}</span>}
                </button>
              );
            })}
          </div>
      ...
      {data.format === "league_knockout" && (
        <Field label="Teams qualifying per group" hint="How many teams from each group advance to the knockout stage.">
          <Input
            type="number"
            min={1}
            max={8}
            value={data.teams_qualify_per_group}
            onChange={e => setData(d => ({ ...d, teams_qualify_per_group: parseInt(e.target.value) || 2 }))}
          />
        </Field>
      )}
      ...
      {showKnockoutOptions && (
        <div className="bg-white border border-outline-variant rounded-2xl p-5 space-y-4">
          <p className="font-bold text-foreground flex items-center gap-2">
            <Icon name="account_tree" className="text-primary" /> Knockout Options
          </p>
          <Toggle
            ...
            onChange={v => setData(d => ({ ...d, super_over_in_knockout: v }))}
          />
          <Toggle
...
function Step3({ data, setData }) {
  ...
  function addPrize() {
    updatePrizes([...prizes, { position: prizes.length + 1, label: "", prize_type: "cash", cash_amount: "", description: "" }]);
  }
  ...
  function updatePrize(i, field, value) {
    updatePrizes(prizes.map((p, idx) => idx === i ? { ...p, [field]: value } : p));
  }
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-4 pt-2 border-t border-outline-variant">
            <div className="grid grid-cols-2 gap-4">
              <Field label="Amount per team">
                <Input
                  ...
                  onChange={e => setData(d => ({ ...d, entry_fee_amount: parseInt(e.target.value) || 0 }))}
                />
              </Field>
              <Field label="Currency">
                <Input
                  ...
                  onChange={e => setData(d => ({ ...d, entry_fee_currency: e.target.value }))}
                  maxLength={10}
                />
              </Field>
            </div>
            <Field label="Fee notes" hint="Shown to team captains — who to pay, how, etc.">
              <Input
                placeholder="e.g. Pay to Ali Khan before first match"
                value={data.entry_fee_notes}
                onChange={e => setData(d => ({ ...d, entry_fee_notes: e.target.value }))}
                maxLength={255}
              />
            </Field>
            <Field label="Fee deadline" hint="Optional reminder date">
              <Input
                ...
                onChange={e => setData(d => ({ ...d, entry_fee_deadline: e.target.value }))}
              />
            </Field>
          </div>
        )}
      ...
      <div className="bg-white border border-outline-variant rounded-2xl p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="font-bold text-foreground flex items-center gap-2">
              <Icon name="workspace_premium" className="text-amber-500" /> Prize Structure
            </p>
            <p className="text-xs text-foreground-muted mt-0.5">Shown publicly on the tournament page.</p>
          </div>
        ...
        {prizes.length === 0 && (
          <p className="text-xs text-foreground-muted text-center py-4">No prizes added yet. Click "Add Prize" to start.</p>
        )}

        {prizes.map((prize, i) => (
          <div key={i} className="border border-outline-variant rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-foreground-muted uppercase tracking-wide">Prize {i + 1}</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Label">
                <Input placeholder="Winner / Best Batsman" value={prize.label}
                  onChange={e => updatePrize(i, "label", e.target.value)} maxLength={100} />
              </Field>
              <Field label="Type">
                <select value={prize.prize_type} onChange={e => updatePrize(i, "prize_type", e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-outline-variant bg-white text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            {prize.prize_type === "cash" && (
              <Field label="Cash Amount">
                <Input type="number" min={0} placeholder="50000" value={prize.cash_amount}
                  onChange={e => updatePrize(i, "cash_amount", e.target.value)} />
              </Field>
            )}
            <Field label="Description" hint="Optional — shown on public page">
              <Input placeholder="Gold trophy + PKR 50,000" value={prize.description}
                onChange={e => updatePrize(i, "description", e.target.value)} maxLength={255} />
            </Field>
          </div>
        ))}

        <Field label="Prize pool summary" hint="Free text shown publicly if no structured prizes above, or as extra context.">
          <Input
            placeholder="e.g. Winner: PKR 50,000 | Runner-up: PKR 20,000"
            value={data.prize_pool_note}
            onChange={e => setData(d => ({ ...d, prize_pool_note: e.target.value }))}
            maxLength={1000}
          />
        </Field>
      </div>
...
const FORMAT_LABELS = { league: "League (Round-Robin)", knockout: "Knockout", league_knockout: "League + Knockout" };
    ...
    <div className="space-y-5">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3">
        ...
        <p className="text-sm text-amber-800">
          The tournament will be saved as a <strong>draft</strong>. You can add teams and generate fixtures from the manage panel before going live.
        </p>
      </div>
      ...
      <div className="bg-white border border-outline-variant rounded-2xl p-5">
        ...
        {data.format !== "knockout" && (
          <>
            ...
            <ReviewRow label="Points — No Result" value={data.points_no_result} />
            <ReviewRow label="Points — Loss"      value={data.points_loss} />
            <ReviewRow label="Tiebreakers"        value={(data.tiebreaker_rules || []).join(" → ")} />
          </>
        )}
        {data.format !== "league" && (
          <>
            <ReviewRow label="Super Over"         value={data.super_over_in_knockout ? "Yes" : "No"} />
            <ReviewRow label="Third-place playoff" value={data.third_place_playoff ? "Yes" : "No"} />
          </>
        )}
        {data.format === "league_knockout" && (
          <ReviewRow label="Teams qualify per group" value={data.teams_qualify_per_group} />
        )}
      </div>
      ...
      {data.entry_fee_amount > 0 && (
        <div className="bg-white border border-outline-variant rounded-2xl p-5">
          ...
          <ReviewRow label="Amount"    value={`${data.entry_fee_currency} ${data.entry_fee_amount.toLocaleString()} per team`} />
      ...
      {prizes.length > 0 && (
        <div className="bg-white border border-outline-variant rounded-2xl p-5">
          ...
          {prizes.map((p, i) => (
            <ReviewRow key={i} label={p.label || `Prize ${i + 1}`}
              value={p.prize_type === "cash" && p.cash_amount ? `Cash — ${data.entry_fee_currency} ${Number(p.cash_amount).toLocaleString()}` : p.prize_type} />
          ))}
        </div>
      )}
    </div>
...
export default function CreateTournamentPage() {
        ...
        for (const [i, prize] of prizes.entries()) {
          await apiRequest(`/api/tournaments/${tournament.code}/prizes`, {
            ...
            body: {
              ...
              cash_amount: prize.cash_amount ? parseInt(prize.cash_amount) : undefined,
  ...
  return (
    <AppShell title="Create Tournament" subtitle="Set up your tournament in a few steps">
      <div className="max-w-2xl">
        <StepBar current={step} />

        <div className="bg-surface rounded-2xl border border-outline-variant p-6 mb-6">
          <h2 className="font-display text-lg font-bold text-foreground mb-6">{STEPS[step]}</h2>
          {steps[step]}
        </div>
        ...
        <div className="flex gap-3">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button type="button" onClick={submit} disabled={saving}
              ...
              {saving
                ? <><Icon name="sync" className="text-base animate-spin" /> Creating…</>
